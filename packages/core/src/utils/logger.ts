...
function getEnvLogLevel(): LogLevel | undefined {
  const envLevel = process.env.MEMSEARCH_LOG_LEVEL;
...
export function createLogger(module: string, options?: Omit<LoggerOptions, 'prefix'>): Logger {
  const level = options?.level ?? globalLogLevel;
  return new Logger({ ...options, level, prefix: `[memsearch:${module}]` });
}
...
export function withErrorContext<T>(
  fn: () => T,
  errorMessage: string,
  ErrorClass: new (msg: string, code?: string, cause?: unknown) => MemSearchError,
  errorCode?: string
): T {
  try {
    return fn();
  } catch (error) {
    throw new ErrorClass(errorMessage, errorCode, error);
  }
}
